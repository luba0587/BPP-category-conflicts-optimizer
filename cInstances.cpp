...
void Instances::load() {
	...
	//se abrir com sucesso, lê e armazena os dados do arquivo
		...
		dataFile >> input;
		binCapacity = binCapacity*Cap_factor;
...
//método que resolve o problema relaxado
void Instances::solveRelaxedProblem()
{
	...
	lowerBound = totalWeight / binCapacity;
}


#ifdef M_GRANDE
//método que monta o problema no Gurobi e o resolve, retornando a solução encontrada
vector<int> Instances::findOptimum() {
	...
	try {
		...
		//Insere limite de tempo para rodar otimização
